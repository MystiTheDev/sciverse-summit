...
> - 🪑 **Chairs download:** `SciVerse-Summit-Chair-Setup-3.0.3.exe` — hosts the session server on your PC (**NOTE: When you install the app for the first time, there will be no account existing. To setup your chair account, login as admin with these credentials: Username: admin  Password: admin123. Then you will be prompted to make a chair account. Enter your details and your first chair account is successfully created 🎉)**
> - 🎤 **Delegates download:** `SciVerse-Summit-Delegate-Setup-3.0.3.exe` — connects to the chair's session
> - **RUN THEM AS ADMINISTRATOR OR ELSE IT WILL NOT WORK!!!**
